				...
				{:else}
					<div class="w-full">
						<div class="flex {($settings?.chatBubble ?? true) ? 'justify-end pb-1' : 'w-full'}">
							...
							>
								{#if message.content}
									<!-- New div for collapsible content -->
						...
						<div
							class="flex items-center space-x-1 text-xs text-gray-400 {($settings?.chatBubble ??
							true)
								? 'justify-end pr-4 -mt-1'
								: 'justify-start -mt-2'}"
						>
							{#if !($settings?.chatBubble ?? true)}
							...
							{#if !readOnly}
								...
								<!-- Add collapse/expand button here -->
								<Tooltip
									content={isCollapsed ? $i18n.t('Expand') : $i18n.t('Collapse')}
									placement="bottom"
								>
									<button
...
<style>
	/* CSS for collapse transition */
	.collapsible-content {
		overflow: hidden;
		transition: max-height 0.3s ease-out; /* Adjust duration and timing function */
	}
	...
	/* Set a large max-height when not collapsed */
	.collapsible-content:not(.collapsed) {
		max-height: 10000px; /* Use a value large enough to accommodate the longest possible message */
	}
</style>
