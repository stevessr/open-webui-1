					...
					>
						...
						>
							{#if (items.find((item) => item.model?.owned_by === 'ollama') && items.find((item) => item.model?.owned_by === 'openai')) || items.find((item) => item.model?.direct) || tags.length > 0}
								<button
									class="min-w-fit outline-none py-2 px-3 {selectedTag === '' &&
							...
							{#if items.find((item) => item.model?.owned_by === 'ollama') && items.find((item) => item.model?.owned_by === 'openai')}
								<button
									class="min-w-fit outline-none py-2 px-3 {selectedConnectionType === 'ollama'
										? ''
										: 'text-gray-300 dark:text-gray-600 hover:text-gray-700 dark:hover:text-white'} transition capitalize"
									on:click={() => {
										selectedTag = '';
								...
								<button
									class="min-w-fit outline-none py-2 px-3 {selectedConnectionType === 'openai'
										? ''
										: 'text-gray-300 dark:text-gray-600 hover:text-gray-700 dark:hover:text-white'} transition capitalize"
									on:click={() => {
										selectedTag = '';
							...
							{#if items.find((item) => item.model?.direct)}
								<button
									class="min-w-fit outline-none py-2 px-3 {selectedConnectionType === 'direct'
										? ''
										: 'text-gray-300 dark:text-gray-600 hover:text-gray-700 dark:hover:text-white'} transition capitalize"
									on:click={() => {
										selectedTag = '';
							...
							{#each tags as tag}
								<button
									class="min-w-fit outline-none py-2 px-3 {selectedTag === tag
										? ''
										: 'text-gray-300 dark:text-gray-600 hover:text-gray-700 dark:hover:text-white'} transition capitalize"
									on:click={() => {
										selectedConnectionType = '';
					...
					>
						<div class="flex flex-col">
							...
							<div class="flex items-center gap-2">
								...
								{#if !$mobile && (item?.model?.tags ?? []).length > 0}
									<div
										class="flex flex-wrap gap-0.5 self-center items-center h-full translate-y-[0.5px]"
									>
										{#each item.model?.tags.sort((a, b) => a.name.localeCompare(b.name)) as tag}
											<Tooltip content={tag.name} className="flex-shrink-0">
												<div
													class=" text-xs font-bold px-1.5 py-0.5 rounded-sm uppercase bg-gray-500/20 text-gray-700 dark:text-gray-200"
												>
													{tag.name}
												</div>
											</Tooltip>
